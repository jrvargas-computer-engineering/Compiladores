...
table_t* tabela_global_obj = NULL;
...
escopo_ini:
    %empty {
        ...
        tabela_global_obj = table_create();
        // scope_stack_push(tabela_global);
        scope_stack_push(tabela_global_obj);
...
chamada_funcao:
    TK_ID '(' lista_argumentos_opcional ')' {
        ...
        /* --- CORREÇÃO AQUI: Calcule o número de argumentos --- */
...
comando_retorno:
    TK_RETORNA expressao TK_ATRIB tipo{
        ...
        // 2. Gera a instrução ILOC: retorna r_temp
        // arg1: registrador com o valor (temp da expressão)
        iloc_node_t* instr_ret = asd_new_iloc(NULL, "retorna", $2->temp, NULL, NULL);
        
        // 3. Adiciona ao final da lista
        $$->code_head = asd_append_instruction($$->code_head, instr_ret);
